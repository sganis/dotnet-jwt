# OpenShift Deployment — Chat-Proxy + Redis
...
1. [Chat-Proxy Environment Variables](#1-chat-proxy-environment-variables)
2. [Redis](#2-redis)
...
## 1. Chat-Proxy Environment Variables
...
```
Chat-Proxy pods  →  redis-rate-limit:6379   (rate limit store)
Chat-Proxy pods  →  LLM-Backend             (inference)
Chat-Proxy pods  →  SEECloud-IIS:443        (JWKS public key fetch)
```

- No public route to LLM-Backend — only Chat-Proxy is exposed via an OpenShift Route
...
Chat-Proxy is fully stateless. Scale replicas freely — Redis enforces all limits globally across pods.
